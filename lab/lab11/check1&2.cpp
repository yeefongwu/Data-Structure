...
int main() {
    ...
    assert (h > e);  // speed beats fire
    assert (h > a);  // speed beats flexible
    assert (h > c);  // speed beats invisible
    assert (b > a);  // flying beats flexible
    assert (!(c > e));  // invisible doesn't beat fire
    assert (d > b);  // laser beats flying
    assert (!(b > f));  // flying doesn't beat wood
    //  */
