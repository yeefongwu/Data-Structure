...
public:   
  ...
  void print() const;
  int getMalloc() const {return m_alloc;}
...
template <class T> int remove_matching_elements(Vec<T>& my_vec, const T& element)
{
    int num_matches = 0;
    //T& Vec<T>::iterator = my_vec.begin();
    T* iterator=my_vec.end();
    while(iterator!=my_vec.begin()){
        iterator--;
        T y=*(iterator);
        if(y==element){
            num_matches+=1;
            my_vec.erase(iterator);
        }
    }
    return num_matches;
}
...
{
    cout << "Allocation size: " << m_alloc
         << "\tSize of vector: " << m_size << endl;
