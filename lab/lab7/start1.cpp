...
int numPaths(vector<vector<bool> > const& blocked_grid, int x, int y, string path){
    int n_paths = 0;
    //cout << "(" << x << ", " << y << ") - ";
    if( (x == 0 && y == 0) ){
        path += "(0, 0)";
    ...
    if( (x > 0) && !(blocked_grid[x-1][y]) ){
        path += "(" + to_string(x) + ", " + to_string(y) + ")-";
        n_paths += numPaths(blocked_grid, x-1, y, path);
    }
    if( (y > 0) && !(blocked_grid[x][y-1]) ){
        path += "(" + to_string(x) + ", " + to_string(y) + ")-";
        n_paths += numPaths(blocked_grid, x, y-1, path);
    }
    return n_paths;
}
...
int main(int argc, char* argv[]) {
    ...
    int p = numPaths(blocked_grid, start_x, start_y, "");
